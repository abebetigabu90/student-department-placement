...
export const GenplacementForOtherSocial = async () => {
    try {
        console.log(chalk.yellow.bold("🔍 Starting placement query..."));
        const OtherSocialDept = await Department.findOne({name: { $regex: /^other\s?social$/i }})
        const OtherSocialStudents = await Student.find({Department: OtherSocialDept._id});
        if (OtherSocialStudents.length === 0) {
        ...
        const sortedStudents = OtherSocialStudents.sort((a, b) => b.totalScore - a.totalScore);
...
export const femaleQuota_AdjustmentForOtherSocial = async () => {
    try {
        console.log('🎯 Starting female quota adjustment for Other Social departments...');
        const OtherSocialDept = await Department.findOne({name: { $regex: /^other\s?social$/i }}) //this code is usefull for later not now
        const departments = await Department.find({
            PrefTimeCategory:'SecSemFirst',PrefTypeCategory:'OtherSocial'
        });
        ...
        for (const dept of departments) {
            try {
                ...
                const femaleStudents = await Student.find({ 
                    gender: { $regex: /^F/i },
                    Department:OtherSocialDept._id
                ...
                for (let i = 0; i < femalesToProcess.length; i++) {
                    ...
                    try {
                        ...
                        await Promise.all([
                            ...
                            Student.findByIdAndUpdate(currentMaleId, { Department: OtherSocialDept._id }),
                            Placement.updateOne({ student: currentFemaleId }, { department: dept._id, priority: 1 }),
                            Placement.updateOne({ student: currentMaleId }, { department: OtherSocialDept._id })
                        ]);                        
                        ...
                        if (!replacementSuccess) {
                            try {
                                await Student.findByIdAndUpdate(currentFemaleId, {Department: OtherSocialDept._id});
                                await Placement.updateOne({student: currentFemaleId},{department: OtherSocialDept._id})
        ...
        console.error(chalk.red.bold('💥 Critical error in femaleQuota_AdjustmentForOtherSocail:'), error);
        throw error;
    }
};
// Placement for Natural Science students who didn't get their first choice
export const placementForUnplacedOtherSocial = async () => {
    try {
        console.log("Starting placement for unplaced other socail Science students...");
        // Find students who need placement
        const OtherSocialDept = await Department.findOne({name: { $regex: /^other\s?social$/i }})
        const unplacedStudents = await Student.find({Department: OtherSocialDept._id});
