  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        setStats({ ...data, placementPercentage });
      } catch (err) {
        console.error(err);
      }
    };
    ...
    <div className="p-6 bg-gray-50 min-h-screen overflow-y-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
        ...
        <StatCard
          ...
          icon={<ClipboardList className="text-yellow-600" size={30} />}
        />
        <StatCard
          title="Female Students"
          value={stats.femaleStudents}
          icon={<Award className="text-pink-500" size={30} />}
        />
        <StatCard
          title="Placement %"
          value={`${stats.placementPercentage}%`}
          icon={<Percent className="text-indigo-600" size={30} />}
        />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <ActionCard
          ...
          desc="Import, edit, or remove student records."
          icon={<UserPlus className="text-indigo-600" size={26} />}
          onClick={() => navigate("/admin/manage-students")}
        />
        <ActionCard
          title="Manage Departments"
          desc="Add or edit department information and capacity."
          icon={<Building2 className="text-teal-600" size={26} />}
          onClick={() => navigate("/admin/manage/departments")}
        />
        <ActionCard
          title="View Reports"
          desc="Check analytics and placement summaries."
          icon={<BarChart3 className="text-blue-700" size={26} />}
          onClick={() => navigate("/admin/view/Reports")}
        />
        <ActionCard
          title="System Settings"
          desc="Adjust placement rules and admin preferences."
          icon={<Settings className="text-gray-700" size={26} />}
          onClick={() => navigate("/admin/preference/setting")}
        />
      </div>
    </div>
  );
