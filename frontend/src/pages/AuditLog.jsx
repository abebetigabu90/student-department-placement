...
//                     <td>
//                       {log.action === "SOFT_DELETE_STUDENT" &&
//                       log.targetId?.isDeleted ? (
//                         <button
//                           onClick={() => handleRestore(log.targetId._id)}
//                           style={{
//                             background: "green",
//                             color: "white",
//                             border: "none",
...
const AuditLogs = () => {
  ...
  const [searchTerm, setSearchTerm] = useState("");
  const [restoringId, setRestoringId] = useState(null); // Track which student is being restored
  ...
  const handleRestore = async (studentId) => {
    if (!studentId || !window.confirm("Are you sure you want to restore this student?")) {
      return;
    }
    ...
    try {
      // Call your restore API endpoint
      await axios.patch(`http://localhost:5000/api/students/${studentId}/restore`,
          {
              userId: userId,
              Role:'admin',
          }
      ...
      // Refresh audit logs to show the update
      fetchAuditLogs();
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Details</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Date & Time</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th> {/* Added column header */}
              </tr>
            </thead>
                ...
                filteredLogs.map((log, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {log.targetId && (
                        <div className="text-xs text-gray-500">
                          ID: { log.targetId.studentId }
                        </div>
                      )}
                    </td>
                    
                    <td className="px-6 py-4">
                      {["UPDATE_STUDENT", "RESTORE_STUDENT", "SOFT_DELETE_STUDENT"].includes(log.action)&&
                      log.changes?.changedFields?.length > 0 ? (
                        <div className="space-y-2">
                          {/* Summary */}
                          <div className="text-sm font-medium text-gray-900">
                            Modified  {log.changes.changedFields.length} field(s)
                          </div>
                    ...
                    {/* Actions Column */}
                    <td className="px-6 py-4 whitespace-nowrap">
                      {log.action === "SOFT_DELETE_STUDENT" || log.action === "DELETE_STUDENT" ? (
                        
                        <button
                            onClick={() => handleRestore(log.targetId._id)}
                            // disabled={restoringId === log.targetId._id || log.changes?.newValues?.isDeleted === false}
                            disabled={log.targetId.isDeleted === false}
                            
                            className={`
                              px-3 py-1.5 rounded-md text-sm font-medium transition
                              ${
                                log.targetId.isDeleted === false
                                  ? "bg-gray-200 text-gray-400 cursor-not-allowed"
                                  : "bg-blue-600 text-white hover:bg-blue-700"
                              }
                            `}
                        >
                          {restoringId === log.targetId._id ? (
